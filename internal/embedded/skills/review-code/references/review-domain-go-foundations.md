...
| AI mode in input.go | If TUI input used, all prompt functions branch on `GlobalForAIFlag`: `PromptInput`/`PromptPassword` use `ReadPipedLine()`, `PromptTextArea` uses `ReadPipedInput()`, `PromptSelect`/`PromptMultiSelect` parse a stdin index / comma-separated index-list | Read `utils/input.go`, check each prompt function for `GlobalForAIFlag` branch |
| input.go exists (if TUI input used) | `PromptInput` (single-line textinput), `PromptPassword` (masked textinput), `PromptTextArea` (multi-line textarea, Ctrl+D submit), `PromptSelect` (single-choice list selector), `PromptMultiSelect` (multi-choice list selector) — all Bubbletea in human mode, piped stdin in AI mode | Glob for `utils/input.go` |
